...
const AetherOnePage = React.lazy(() => Promise.reject()); // Desativado
...
function AppContent() {
  ...
  const [showAuthModal, setShowAuthModal] = useState(false); // Desabilitado
  ...
  const handleNavigate = useCallback((view: ViewType) => {
    ...
    if (view === 'client-portal' && authRole !== 'client' && authRole !== 'admin') {
        // setShowAuthModal(true); // Desabilitado
        setIsMobileMenuOpen(false);
        return;
    }
  ...
  const isFullScreenView = ['admin-dashboard', 'client-portal', 'briefing'].includes(currentView);
    ...
    <HelmetProvider>
        <div className="min-h-screen relative font-sans bg-white dark:bg-dark flex flex-col text-gray-900 dark:text-white overflow-x-hidden transition-colors duration-300">
            ...
            <main className={`flex-grow ${!isFullScreenView ? 'pb-20 md:pb-0' : ''}`}>
                ...
                {/* Zenith e Aether demos removidas */}
                {/* currentView === 'zenith-demo' removido */}
                {/* currentView === 'aether-demo' removido */}
