...
import { MessageCircle, X, Send, CheckSquare, RefreshCcw, Code2, HelpCircle, Briefcase, Info, Clock, ShieldAlert } from 'lucide-react';
...
import { submitChatbotBriefing } from '../lib/contactService';
import { resolveSupportKnowledge } from '../lib/chatbotKnowledgeEngine';

const CHAT_TRIGGERS = [
  'Quer um site que passe mais autoridade?',
  'Posso montar seu briefing em 2 minutos.',
  'Descubra o pacote certo antes de pedir proposta.',
  'Quer entender prazo, valor e estrutura ideal?',
  'Se preferir, eu te levo direto para o WhatsApp.',
  'Vamos transformar sua ideia em proposta clara.',
  'Seu projeto precisa vender melhor na internet?',
  'Posso te ajudar a definir a melhor solucao.',
  'Tem duvida sobre site, landing page ou escopo?',
  'Comece com um briefing rapido e objetivo.'
];

const SUPPORT_KEYWORDS = [
  'ajuda', 'socorro', 'duvida', 'suporte', 'problema',
  'nao entendi', 'como funciona', 'explica'
];
...
export const Chatbot: React.FC<ChatbotProps> = ({ isOpen, setIsOpen, onNavigate, contextService, extraElevation, initialMode }) => {
  ...
  const handleInputSubmit = (e: React.FormEvent) => {
    ...
    if (typeof stepConfig?.validation === 'function' && value.trim()) {
      const validationResult = stepConfig.validation(value);
      if (typeof validationResult === 'object' && !validationResult.isValid) {
        addBotMessage(validationResult.message || 'Revise essa informacao e tente novamente.');
        return;
      }

      if (typeof validationResult === 'object' && validationResult.cleaned) {
        value = validationResult.cleaned;
      }
    }
    ...
    if (currentStepId === 'support_question') {
      const resolution = resolveSupportKnowledge(value, budgetData);
      addBotMessage(resolution.answer, 'summary', resolution.options as ChatOption[]);
      return;
    }
  ...
  const handleBriefingEmail = async () => {
    ...
    try {
      await submitChatbotBriefing(budgetData);

      if (!isMounted.current) return;

      setIsTyping(false);
      processStep('briefing_sent', budgetData);
    } catch (error: any) {
      if (!isMounted.current) return;

      setIsTyping(false);
      addBotMessage(error?.message || 'Nao consegui enviar o briefing agora. Tente novamente ou siga pelo WhatsApp.', 'summary', [
        { label: 'Tentar enviar por e-mail de novo', value: 'email_briefing' },
        { label: 'Enviar no WhatsApp', value: 'finish', nextId: 'finalize' }
      ]);
    } finally {
  ...
  const handleOptionClick = (option: ChatOption) => {
    ...
    if (option.value === 'email_briefing') {
  ...
  const handleWhatsAppShortcut = () => {
    const text = 'Ola. Quero falar direto sobre meu projeto e receber uma orientacao comercial.';
            ...
            <div className={`p-4 flex flex-col gap-3 shadow-md shrink-0 border-b border-gray-800 transition-colors duration-500 ${chatMode === 'support' ? 'bg-indigo-900' : 'bg-gray-900'}`}>
              <div className="flex justify-between items-center text-white">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 flex items-center gap-1 mt-0.5">
                      ...
                      <span className="uppercase tracking-wide">
                        {chatMode === 'support' ? 'Orientacao e duvidas' : 'Briefing comercial'}
                      </span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-1">
                  <button onClick={handleWhatsAppShortcut} className="text-green-400 hover:text-green-300 hover:bg-white/10 p-2 rounded-full transition-colors" title="Falar no WhatsApp">
              ...
              <div className="bg-black/20 p-1 rounded-xl flex gap-1">
                ...
                >
                  <Briefcase size={12} /> Vender melhor
                </button>
                <button
                      ...
                      {msg.type === 'process-info' && (
                        <div className="ml-0 md:ml-8 mt-2 w-[95%] bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                          <div className="bg-primary-50 dark:bg-primary-900/20 p-3 border-b border-primary-100 dark:border-primary-800 flex justify-between items-center">
                            <p className="text-xs font-bold text-primary-800 dark:text-primary-300 uppercase tracking-wider">Como o projeto avanca</p>
      ...
      {!isOpen && !isMobile && (
        <div className={`fixed right-8 z-[90] transition-all duration-300 ${shouldElevate ? 'bottom-24' : 'bottom-8'}`}>
          ...
          <button
            ...
            aria-label="Abrir chatbot comercial"
          >
